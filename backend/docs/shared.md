...
Shared = one place for everything used across multiple modules
...
LoggingInterceptor      ← logs "→ METHOD /url"
  ↓
ParseUuidPipe           ← validates :id param
  ↓
Controller
  ↓ (throws NotFoundException etc.)
HttpExceptionFilter     ← catches all HttpExceptions globally
  ↓
LoggingInterceptor      ← logs "← METHOD /url +Xms"
  ↓
HTTP Response
```
...
```text
DTO field
  @Trim()                              ← from @app/shared/decorators
  @IsString()

Controller
  @Query() query: PaginationQueryDto  ← from @app/shared/dto
  @Param('id', ParseUuidPipe)         ← from @app/shared/pipes

Handler / Repository return type
  Promise<PaginatedResult<Todo>>      ← from @app/shared/types

AppModule
  load: [appConfig]                   ← from @app/shared/config
```
...
Always import from `@app/shared` — never from deep paths like `@app/shared/src/dto/...`

```ts
// ✅
import { Trim, PaginationQueryDto } from '@app/shared';

// ❌
import { Trim } from '@app/shared/src/decorators/trim.decorator';
```

---

## 2. Add to Shared Only When Truly Shared

Something belongs in shared when it is used in **2 or more** feature modules.  
Feature-specific DTOs, interfaces, or decorators stay inside the feature.

---

## 3. No Business Logic in Shared

Shared contains only infrastructure-level utilities.  
Domain concepts never live here.

---

# Summary

- Shared = reusable config, decorators, DTOs, filters, interceptors, pipes, and types
- Single entry point: `import { ... } from '@app/shared'`
- `appConfig` — typed env config with defaults, loaded globally in `AppModule`
- `@Trim()` — always the first decorator on string DTO fields
- `PaginationQueryDto` — standard pagination params for all list endpoints
- `HttpExceptionFilter` — registered globally in `main.ts`, never per-controller
- `LoggingInterceptor` — registered globally in `main.ts`, logs every request with timing
- `ParseUuidPipe` — applied to every `:id` route param, fails fast on invalid UUID
- `PaginatedResult<T>` — generic return type for all paginated repository and handler methods
